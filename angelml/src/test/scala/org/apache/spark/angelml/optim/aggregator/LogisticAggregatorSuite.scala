...
import org.apache.spark.angelml.linalg.{BLAS, Matrices, Vector, Vectors}
...
class LogisticAggregatorSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("check sizes multinomial") {
    ...
    val numFeatures = instances.head.features.size.toInt
  ...
  test("check sizes binomial") {
    ...
    val numFeatures = binaryInstances.head.features.size.toInt
