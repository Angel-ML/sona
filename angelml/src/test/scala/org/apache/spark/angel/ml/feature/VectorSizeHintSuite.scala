...
class VectorSizeHintSuite
  ...
  test("Required params must be set before transform.") {
    ...
    testTransformerByInterceptingException[(linalg.Vector, Int)](
      data,
      noSizeTransformer,
  ...
  test("Adding size to column of vectors.") {
    ...
    for (handleInvalid <- VectorSizeHint.supportedHandleInvalids) {
      ...
      testTransformerByGlobalCheckFunc[Tuple1[linalg.Vector]](dataFrame, transformer, vectorColName) {
        rows => {
          assert(
  ...
  test("Size hint preserves attributes.") {
    ...
    for (handleInvalid <- VectorSizeHint.supportedHandleInvalids) {
      val transformer = new VectorSizeHint()
        ...
        .setSize(size)
        .setHandleInvalid(handleInvalid)
      testTransformerByGlobalCheckFunc[(Int, Int, Int, linalg.Vector)](
  ...
  test("Size mismatch between current and target size raises an error.") {
    ...
    for (handleInvalid <- VectorSizeHint.supportedHandleInvalids) {
      val transformer = new VectorSizeHint()
        ...
        .setSize(size)
        .setHandleInvalid(handleInvalid)
      testTransformerByInterceptingException[(Int, Int, Int, linalg.Vector)](
        dataFrameWithMetadata,
        transformer,
  ...
  test("Handle invalid does the right thing.") {
    ...
    testTransformerByInterceptingException[Tuple1[linalg.Vector]](
      dataWithNull,
      sizeHint,
    ...
    testTransformerByGlobalCheckFunc[Tuple1[linalg.Vector]](dataWithNull, sizeHint, "vector") { rows =>
      assert(rows.length === 1)
    }
    testTransformerByGlobalCheckFunc[Tuple1[linalg.Vector]](dataWithShort, sizeHint, "vector") { rows =>
      assert(rows.length === 1)
    }
