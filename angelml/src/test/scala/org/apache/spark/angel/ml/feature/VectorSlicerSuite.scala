...
import org.apache.spark.angel.ml.attribute.{Attribute, AttributeGroup, NumericAttribute}
...
class VectorSlicerSuite extends MLTest with DefaultReadWriteTest {
  ...
  test("Test vector slicer") {
    ...
    def validateResults(rows: Seq[Row]): Unit = {
      rows.foreach { case Row(vec1: linalg.Vector, vec2: linalg.Vector) =>
        assert(vec1 === vec2)
      }
    ...
    testTransformerByGlobalCheckFunc[(linalg.Vector, linalg.Vector)](df, vectorSlicer, "result", "expected")(
      validateResults)
