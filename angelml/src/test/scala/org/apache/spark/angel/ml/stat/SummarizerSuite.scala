...
class SummarizerSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  private case class ExpectedMetrics(
                                      ...
                                      count: Long,
                                      numNonZeros: linalg.Vector,
                                      max: linalg.Vector,
                                      min: linalg.Vector,
                                      normL2: linalg.Vector,
                                      normL1: linalg.Vector)
  ...
  /**
    ...
    */
  private def testExample(name: String, inputVec: Seq[(linalg.Vector, Double)],
                          exp: ExpectedMetrics, expWithoutWeight: ExpectedMetrics): Unit = {

    val summarizer = {
      val _summarizer = new MultivariateOnlineSummarizer
      inputVec.foreach(v => _summarizer.add(v._1, v._2))
      _summarizer
    }

    val summarizerWithoutWeight = {
      val _summarizer = new MultivariateOnlineSummarizer
      inputVec.foreach(v => _summarizer.add(v._1))
      _summarizer
    }
  ...
  test("no element") {
    val df = Seq[Tuple1[linalg.Vector]]().toDF("features")
  ...
  ignore("performance test") {
    ...
    */
    ...
    for ((n, dim) <- Seq(
      ...
      val rddFromDf = df.rdd.map { case Row(v: linalg.Vector) => v }
      for (i <- 1 to numOfTry) {
        val start = System.nanoTime()
