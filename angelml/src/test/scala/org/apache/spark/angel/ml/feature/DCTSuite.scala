...
@BeanInfo
case class DCTTestData(vec: linalg.Vector, wantedVec: linalg.Vector)
...
class DCTSuite extends MLTest with DefaultReadWriteTest {
  ...
  private def testDCT(data: linalg.Vector, inverse: Boolean): Unit = {
    ...
    testTransformer[(linalg.Vector, linalg.Vector)](dataset, transformer, "resultVec", "wantedVec") {
      case Row(resultVec: linalg.Vector, wantedVec: linalg.Vector) =>
