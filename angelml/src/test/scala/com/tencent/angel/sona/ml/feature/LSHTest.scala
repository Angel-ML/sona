...
import com.tencent.angel.sona.ml.util.MLTestingUtils
...
private[angel] object LSHTest {
  ...
  def calculateLSHProperty[T <: LSHModel[T]](
    ...
    val headHashValue = transformedData.select(outputCol).head().get(0).asInstanceOf[Seq[linalg.Vector]]
    ...
    val distUDF = udf((x: linalg.Vector, y: linalg.Vector) => model.keyDistance(x, y), DataTypes.DoubleType)
    val sameBucket = udf((x: Seq[linalg.Vector], y: Seq[linalg.Vector]) => model.hashDistance(x, y) == 0.0,
  ...
  def calculateApproxNearestNeighbors[T <: LSHModel[T]](
    ...
    // Compute expected
    val distUDF = udf((x: linalg.Vector) => model.keyDistance(x, key), DataTypes.DoubleType)
  ...
  def calculateApproxSimilarityJoin[T <: LSHModel[T]](
    ...
    assert(
      DataTypeUtil.sameType(actual.schema.apply("datasetB").dataType,
