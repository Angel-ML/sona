...
import org.apache.spark.linalg.{IntSparseVector, Vectors}
...
class VectorIndexerSuite extends MLTest with DefaultReadWriteTest with Logging {
  ...
  override def beforeAll(): Unit = {
    ...
    def checkPair(dvSeq: Seq[linalg.Vector], svSeq: Seq[linalg.Vector]): Unit = {
  ...
  test("Cannot fit an empty DataFrame") {
    val rdd = Array.empty[linalg.Vector].map(FeatureData).toSeq.toDF()
  ...
  test("Builds valid categorical feature value index, transform correctly, check metadata") {
    def checkCategoryMaps(
      ...
      val collectedData = data.collect().map(_.getAs[linalg.Vector](0))
      ...
      try {
        ...
        testTransformerByGlobalCheckFunc[FeatureData](data, model, "indexed") { rows =>
          val transformed = rows.map { r => Tuple1(r.getAs[linalg.Vector](0)) }.toDF("indexed")
          val indexedRDD: RDD[linalg.Vector] = transformed.rdd.map(_.getAs[linalg.Vector](0))
  ...
  test("Maintain sparsity for sparse vectors") {
    def checkSparsity(data: DataFrame, maxCategories: Int): Unit = {
      val points = data.collect().map(_.getAs[linalg.Vector](0))
      val vectorIndexer = getIndexer.setMaxCategories(maxCategories)
      val model = vectorIndexer.fit(data)
      testTransformerByGlobalCheckFunc[FeatureData](data, model, "indexed") { rows =>
        points.zip(rows.map(_ (0))).foreach {
          case (orig: IntSparseVector, indexed: IntSparseVector) =>
...
private[sona] object VectorIndexerSuite {

  @BeanInfo
  case class FeatureData(@BeanProperty features: linalg.Vector)
