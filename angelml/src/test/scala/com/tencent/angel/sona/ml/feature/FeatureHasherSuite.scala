...
class FeatureHasherSuite extends MLTest with DefaultReadWriteTest {
  ...
  implicit private val vectorEncoder: ExpressionEncoder[linalg.Vector] = ExpressionEncoder[linalg.Vector]()
  ...
  test("feature hashing") {
    ...
    testTransformer[(Double, Boolean, String, String, linalg.Vector)](df, hasher, "features", "expected") {
  ...
  test("setting explicit numerical columns to treat as categorical") {
    ...
    val features = output.select("features").as[linalg.Vector].collect()
  ...
  test("hashing works for all numeric types") {
    ...
    val expectedResult = hasher.transform(df).select("features").as[linalg.Vector].collect()
    ...
    types.foreach { t =>
      ...
      val castResult = hasher.transform(castDF).select("features").as[linalg.Vector].collect()
  ...
  test("hash collisions sum feature values") {
    ...
    val features = hasher.transform(df).select("features").as[linalg.Vector].collect()
    def idx: Any => Int = murmur3FeatureIdx(n)
...
object FeatureHasherSuite {
  ...
  private[sona] def murmur3FeatureIdx(numFeatures: Int)(term: Any): Int = {
    SparkUtil.nonNegativeMod(FeatureHasher.murmur3Hash(term), numFeatures)
  }
