...
import com.tencent.angel.sona.ml.feature.{Instance, LabeledPoint}
...
object MLTestingUtils extends SparkFunSuite {
  ...
  def genEquivalentOversampledAndWeightedInstances(
    ...
    val overSampledData = rawData.rdd.flatMap { case Row(label: Double, features: linalg.Vector, n: Int) =>
      Iterator.fill(n)(Instance(label, 1.0, features))
    }.toDS()
    rng.setSeed(seed)
    val weightedData = rawData.rdd.map { case Row(label: Double, features: linalg.Vector, n: Int) =>
  ...
  def generateArrayFeatureDataset(dataset: Dataset[_],
    ...
    val toFloatVectorUDF = udf { (features: linalg.Vector) =>
      Vectors.dense(features.toArray.map(_.toFloat.toDouble))
    }
    val toDoubleArrayUDF = udf { (features: linalg.Vector) => features.toArray }
    val toFloatArrayUDF = udf { (features: linalg.Vector) => features.toArray.map(_.toFloat) }
