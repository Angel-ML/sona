...
class MinHashLSHSuite extends MLTest with DefaultReadWriteTest {
  ...
  test("hashFunction: empty vector") {
    ...
    intercept[IllegalArgumentException] {
      model.hashFunction(Vectors.sparse(10, Seq[(Int, Double)]()))
    }
  }
  ...
  test("approxNearestNeighbors for min hash") {
    ...
    val key: linalg.Vector = Vectors.sparse(100,
  ...
  test("MinHashLSHModel.transform should work with Structured Streaming") {
    ...
    testTransformer[Tuple1[linalg.Vector]](dataset.toDF(), model, "keys", model.getOutputCol) {
      case Row(_: linalg.Vector, output: Seq[_]) =>
        ...
        output.foreach {
          case hashOutput: linalg.Vector => assert(hashOutput.size === 1)
        }
    }
  }
}
