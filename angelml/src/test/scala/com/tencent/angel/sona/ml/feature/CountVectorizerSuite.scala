...
class CountVectorizerSuite extends MLTest with DefaultReadWriteTest {
  ...
  test("CountVectorizerModel common cases") {
    val df = Seq(
      ...
      (3, split(""), Vectors.sparse(4, Seq[(Int, Double)]())), // empty string
    ...
    testTransformer[(Int, Seq[String], linalg.Vector)](df, cv, "features", "expected") {
      case Row(features: linalg.Vector, expected: linalg.Vector) =>
  ...
  test("CountVectorizer common cases") {
    ...
    testTransformer[(Int, Seq[String], linalg.Vector)](df, cvm, "features", "expected") {
      case Row(features: linalg.Vector, expected: linalg.Vector) =>
        assert(features ~== expected absTol 1e-14)
    }
  }
  ...
  test("CountVectorizer vocabSize and minDF") {
    ...
    testTransformer[(Int, Seq[String], linalg.Vector)](df, cvModel2, "features", "expected") {
      case Row(features: linalg.Vector, expected: linalg.Vector) =>
        assert(features ~== expected absTol 1e-14)
    }
    ...
    testTransformer[(Int, Seq[String], linalg.Vector)](df, cvModel3, "features", "expected") {
      case Row(features: linalg.Vector, expected: linalg.Vector) =>
        assert(features ~== expected absTol 1e-14)
    }
  }
  ...
  test("CountVectorizer maxDF") {
    val df = Seq(
      ...
      (3, split("a"), Vectors.sparse(3, Seq[(Int, Double)]()))
    ).toDF("id", "words", "expected")
  ...
  test("CountVectorizer using both minDF and maxDF") {
    ...
    val df = Seq(
      ...
      (3, split("a"), Vectors.sparse(2, Seq[(Int, Double)]()))
    ).toDF("id", "words", "expected")
  ...
  test("CountVectorizerModel with minTF count") {
    val df = Seq(
      ...
      (2, split("a"), Vectors.sparse(4, Seq[(Int, Double)]())),
      (3, split("e e e e e"), Vectors.sparse(4, Seq[(Int, Double)]()))
    ).toDF("id", "words", "expected")
  ...
  test("CountVectorizerModel and CountVectorizer with binary") {
    ...
    val cv2 = new CountVectorizerModel(cv.vocabulary)
      ...
      .setOutputCol("features")
      .setBinary(true)
    testTransformer[(Int, Seq[String], linalg.Vector)](df, cv2, "features", "expected") {
      case Row(features: linalg.Vector, expected: linalg.Vector) =>
        assert(features ~== expected absTol 1e-14)
    }
  }
