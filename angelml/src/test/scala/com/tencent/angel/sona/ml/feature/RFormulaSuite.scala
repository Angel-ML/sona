...
//      assert(rows.head.schema.toString() == resultSchema.toString())
  ...
  test("vector attribute generation") {
    ...
    testRFormulaTransform[(Int, linalg.Vector)](original, model, expected, expectedAttrs)
  }
  ...
  test("Use Vectors as inputs to formula.") {
    ...
    testTransformer[(Int, Int, linalg.Vector)](original, formula.fit(original), first, rest: _*) {
      case Row(id: Int, a: Int, b: linalg.Vector, features: linalg.Vector, label: Double) =>
        assert(label === id)
    ...
    testTransformer[(Int, Int, linalg.Vector)](original, model, first, rest: _*) {
      case Row(id: Int, a: Int, b: linalg.Vector, features: linalg.Vector, label: Double) =>
        assert(label === id)
        assert(features.toArray === a +: b.toArray)
    }
