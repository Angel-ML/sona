...
import org.apache.spark.linalg.SQLDataTypes.VectorType
import org.apache.spark.linalg.{DenseVector, IntSparseVector, LongSparseVector, Vector, Vectors}
...
//    val v = row1.getAs[IntSparseVector](1)
  ...
  test("write libsvm data and read it again") {
    val df = spark.read.format("libsvmex").load(path)
    val writePath = SparkUtil.createTempDir().getPath
...
//  }
  ...
  test("create libsvmTable table without schema and path") {
    try {
      val e = intercept[IllegalArgumentException] {
        spark.sql("CREATE TABLE libsvmTable USING libsvmex")
      }
