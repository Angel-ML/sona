...
import org.apache.spark.sql.execution.datasources.{HadoopFsRelation, OutputWriter}
import org.apache.spark.sql.types.StructType


/**
  * A factory that produces [[datasources.OutputWriter]]s.  A new [[OutputWriterFactory]] is created on driver
  * side for each write job issued when writing to a [[HadoopFsRelation]], and then gets serialized
  * to executor side to create actual [[datasources.OutputWriter]]s on the fly.
  */
abstract class OutputWriterFactory extends Serializable {
  ...
  /**
    ...
    * to instantiate new [[datasources.OutputWriter]]s.
    *
...
abstract class OutputWriter {
  ...
  /**
    * Closes the [[datasources.OutputWriter]]. Invoked on the executor side after all rows are persisted, before
