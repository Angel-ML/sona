...
import org.apache.spark.angel.ml.attribute.{Attribute, AttributeGroup, BinaryAttribute, NominalAttribute, NumericAttribute, UnresolvedAttribute}
import org.apache.spark.angel.ml.linalg.{Vector, VectorUDT, Vectors}
import org.apache.spark.angel.ml.param.ParamMap
import org.apache.spark.angel.ml.param.shared.{HasInputCols, HasOutputCol}
  ...
  def foreachNonzeroOutput(value: Any, f: (Int, Double) => Unit): Unit = value match {
    ...
    case vec: Vector =>
      ...
      vec.foreachActive { (i, v) =>
        val numOutputCols = numFeatures(i.toInt)
        if (numOutputCols > 1) {
          assert(
            v >= 0.0 && v == v.toInt && v < numOutputCols,
            s"Values from column must be indices, but got $v.")
          f(outputOffsets(i.toInt) + v.toInt, 1.0)
        } else {
          f(outputOffsets(i.toInt), v)
        }
      }
    case null =>
