...
import org.apache.spark.angel.ml.linalg.{Matrix, Vectors}
...
private[stat] object SpearmanCorrelation extends Correlation with Logging {
  ...
  /**
    ...
    */
  override def computeCorrelationMatrix(X: RDD[linalg.Vector]): Matrix = {
    // ((columnIndex, value), rowUid)
    val colBased = X.zipWithUniqueId().flatMap{ case (vec: linalg.Vector, uid: Long) =>
