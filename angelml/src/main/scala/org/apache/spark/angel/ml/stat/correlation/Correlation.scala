...
package org.apache.spark.angel.ml.stat.correlation

import org.apache.spark.angel.ml.linalg
import org.apache.spark.angel.ml.linalg.{DenseVector, Matrix}
...
private[stat] trait Correlation {
   ...
   */
  def computeCorrelationMatrix(X: RDD[linalg.Vector]): Matrix
  ...
  def computeCorrelationWithMatrixImpl(x: RDD[Double], y: RDD[Double]): Double = {
    val mat: RDD[linalg.Vector] = x.zip(y).map { case (xi, yi) => new DenseVector(Array(xi, yi)) }
...
private[stat] object Correlations {
  ...
  def corrMatrix(X: RDD[linalg.Vector],
 ...
 */
private[angelml] object CorrelationNames {
