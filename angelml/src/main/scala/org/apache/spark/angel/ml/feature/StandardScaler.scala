...
import org.apache.spark.angel.ml.linalg.{DenseVector, IntSparseVector, VectorUDT, Vectors}
import org.apache.spark.angel.ml.param.{BooleanParam, ParamMap, Params}
import org.apache.spark.angel.ml.param.shared.{HasInputCol, HasOutputCol}
import org.apache.spark.angel.ml.stat.MultivariateOnlineSummarizer
...
class StandardScaler @Since("1.4.0") (
  ...
  override def fit(dataset: Dataset[_]): StandardScalerModel = {
    ...
    val input: RDD[ml.linalg.Vector] = dataset.select($(inputCol)).rdd.map {
      case Row(v: ml.linalg.Vector) => v
    }
...
class StandardScalerModel private[angelml](
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val transformer: ml.linalg.Vector => ml.linalg.Vector = vector => {
      ...
      if ($(withMean)) {
        ...
        val values = vector match {
          ...
          case v: ml.linalg.Vector => v.toArray
        }
...
object StandardScalerModel extends MLReadable[StandardScalerModel] {
  ...
  class StandardScalerModelWriter(instance: StandardScalerModel) extends MLWriter {
    ...
    private case class Data(std: ml.linalg.Vector, mean: ml.linalg.Vector)
  ...
  private class StandardScalerModelReader extends MLReader[StandardScalerModel] {
    ...
    override def load(path: String): StandardScalerModel = {
      ...
      val Row(std: ml.linalg.Vector, mean: ml.linalg.Vector) = MLUtils.convertVectorColumnsToML(data, "std", "mean")
        .select("std", "mean")
        .head()
