...
import org.apache.spark.angel.ml.linalg.BLAS.axpy
...
object LBFGS extends Logging {
  ...
  private class CostFun(
    ...
    override def calculate(weights: BDV[Double]): (Double, BDV[Double]) = {
      ...
      val seqOp = (c: (linalg.Vector, Double), v: (Double, linalg.Vector)) =>
        (c, v) match {
      ...
      val combOp = (c1: (linalg.Vector, Double), c2: (linalg.Vector, Double)) =>
      ...
      val zeroSparseVector = Vectors.sparse(n, Seq.empty[(Int, Double)])
