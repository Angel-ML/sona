...
import org.apache.spark.angel.ml.attribute.{Attribute, BinaryAttribute, NominalAttribute, NumericAttribute, UnresolvedAttribute}
import org.apache.spark.angel.ml.linalg.Vectors
import org.apache.spark.angel.ml.param.{Param, ParamMap, ParamPair, Params}
import org.apache.spark.angel.ml.param.shared.{HasParallelism, HasWeightCol}
import org.apache.spark.annotation.Since
import org.apache.spark.angel.ml.util.Instrumentation.instrumented
import org.apache.spark.angel.ml.util.{DefaultParamsReader, DefaultParamsWriter, Identifiable, MLReadable, MLReader, MLWritable, MLWriter, MetadataUtils}
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val aggregatedDataset = models.zipWithIndex.foldLeft[DataFrame](newDataset) {
      case (df, (model, index)) =>
        ...
        val updateUDF = udf { (predictions: Map[Int, Double], prediction: ml.linalg.Vector) =>
    ...
    if (getRawPredictionCol != "") {
      ...
      val labelUDF = udf { (rawPredictions: ml.linalg.Vector) => rawPredictions.argmax.toDouble }
