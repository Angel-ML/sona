...
import org.apache.spark.angel.ml.feature.{Instance, StandardScaler}
import org.apache.spark.angel.ml.linalg
import org.apache.spark.angel.ml.linalg.DenseVector
 ...
 *
 * However, we don't want to apply the [[StandardScaler]] on the
...
private[angelml] class LogisticAggregator(
    ...
    multinomial: Boolean)(bcCoefficients: Broadcast[linalg.Vector])
  extends DifferentiableLossAggregator[Instance, LogisticAggregator] with Logging {

  private val numFeatures = bcFeaturesStd.value.length
  private val numFeaturesPlusIntercept = if (fitIntercept) numFeatures + 1 else numFeatures
  private val coefficientSize = bcCoefficients.value.size.toInt
  ...
  private def binaryUpdateInPlace(features: linalg.Vector, weight: Double, label: Double): Unit = {
    ...
    features.foreachActive { (index, value) =>
      if (localFeaturesStd(index.toInt) != 0.0 && value != 0.0) {
        localGradientArray(index.toInt) += multiplier * value / localFeaturesStd(index.toInt)
      }
    }
  ...
  private def multinomialUpdateInPlace(features: linalg.Vector, weight: Double, label: Double): Unit = {
    ...
    features.foreachActive { (index, value) =>
      if (localFeaturesStd(index.toInt) != 0.0 && value != 0.0) {
        val stdValue = value / localFeaturesStd(index.toInt)
        var j = 0
        while (j < numClasses) {
          margins(j) += localCoefficients(index.toInt * numClasses + j) * stdValue
          j += 1
        }
      }
    }
    ...
    features.foreachActive { (index, value) =>
      if (localFeaturesStd(index.toInt) != 0.0 && value != 0.0) {
        ...
        var j = 0
        while (j < numClasses) {
          localGradientArray(index.toInt * numClasses + j) += weight * multipliers(j) * stdValue
          j += 1
        }
      }
    }
