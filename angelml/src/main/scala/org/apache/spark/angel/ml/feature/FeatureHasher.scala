...
import org.apache.spark.angel.ml.param.{BooleanParam, ParamMap, StringArrayParam}
import org.apache.spark.angel.ml.param.shared.{HasInputCols, HasNumFeatures, HasOutputCol}
import org.apache.spark.angel.ml.util.{DefaultParamsReadable, DefaultParamsWritable, Identifiable, SchemaUtils}
      ...
      udf { row: Row =>
        ...
        localInputCols.foreach { colName =>
          ...
          if (!row.isNullAt(fieldIndex)) {
            ...
            val idx = Utils.nonNegativeMod(rawIdx, n.toInt)
...
object FeatureHasher extends DefaultParamsReadable[FeatureHasher] {
  ...
  private[feature] def murmur3Hash64(term: Any): Long = {
    val str = term match {
      case str: String => str
