...
import org.apache.spark.angel.ml.param.{IntParam, ParamMap, ParamValidators, Params}
...
object IDF extends DefaultParamsReadable[IDF] {
  ...
  class DocumentFrequencyAggregator(val minDocFreq: Int) extends Serializable {
    ...
    /** Adds a new document. */
    def add(doc: ml.linalg.Vector): this.type = {
      if (isEmpty) {
...
class IDFModel private[angelml](
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val idfUDF = udf { vec: ml.linalg.Vector => IDFModel.transform(bcIdf.value, vec) }
...
object IDFModel extends MLReadable[IDFModel] {
  ...
  private[IDFModel] class IDFModelWriter(instance: IDFModel) extends MLWriter {
    ...
    private case class Data(idf: ml.linalg.Vector)
  ...
  private class IDFModelReader extends MLReader[IDFModel] {
    ...
    override def load(path: String): IDFModel = {
      ...
      val Row(idf: ml.linalg.Vector) = MLUtils.convertVectorColumnsToML(data, "idf")
        .select("idf")
        .head()
  ...
  /**
    ...
    */
  def transform(idf: ml.linalg.Vector, v: ml.linalg.Vector): ml.linalg.Vector = {
    val n = v.size.toInt
    v match {
