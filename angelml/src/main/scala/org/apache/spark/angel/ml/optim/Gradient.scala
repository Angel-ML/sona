...
import org.apache.spark.angel.ml.linalg.{DenseVector, Vectors}
import org.apache.spark.angel.ml.util.MLUtils
import org.apache.spark.annotation.DeveloperApi
import org.apache.spark.angel.ml.linalg.BLAS.{axpy, dot, scal}
...
abstract class Gradient extends Serializable {
  /**
    ...
    */
  def compute(data: linalg.Vector, label: Double, weights: linalg.Vector): (linalg.Vector, Double) = {
    val gradient = Vectors.zeros(weights.size.toInt)
  ...
  /**
    ...
    * @return loss
    */
  def compute(data: linalg.Vector, label: Double, weights: linalg.Vector, cumGradient: linalg.Vector): Double
}
      ...
      case _ =>
        ...
        val margins = Array.tabulate(numClasses - 1) { i =>
          ...
          data.foreachActive { (index, value) =>
            if (value != 0.0) margin += value * weightsArray((i * dataSize) + index.toInt)
          }
        ...
        for (i <- 0 until numClasses - 1) {
          ...
          data.foreachActive { (index, value) =>
            if (value != 0.0) cumGradientArray(i * dataSize + index.toInt) += multiplier * value
          }
        }
...
class HingeGradient extends Gradient {
  override def compute(data: linalg.Vector, label: Double, weights: linalg.Vector): (linalg.Vector, Double) = {
    ...
    if (1.0 > labelScaled * dotProduct) {
      ...
      (Vectors.sparse(weights.size, Array.empty[Int], Array.empty[Double]), 0.0)
    }
  }
