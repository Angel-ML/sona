...
object MLUtils extends Logging {
  ...
  private[spark] def computeLongKeyNumFeatures(rdd: RDD[(Double, Array[Long], Array[Double])]): Long = {
  ...
  private[spark] def parseLongKeyLibSVMRecord(line: String): (Double, Array[Long], Array[Double]) = {
    val items = line.split(' ')
    val label = items.head.toDouble
    val (indices, values) = items.tail.filter(_.nonEmpty).map { item =>
      val indexAndValue = item.split(':')
...
//    }
  ...
  /**
    * Converts vector columns in an input Dataset from the [[Vector]]
    * type to the new [[org.apache.spark.angelml.linalg.Vector]] type under the `spark.ml` package.
    *
  ...
  def convertVectorColumnsToML(dataset: Dataset[_], cols: String*): DataFrame = {
    ...
    val convertToML = udf { v: Vector => v }
  ...
  /**
    * Converts vector columns in an input Dataset to the [[Vector]]
    * type from the new [[org.apache.spark.angelml.linalg.Vector]] type under the `spark.ml` package.
    *
    * @param dataset input dataset
  ...
  /**
    ...
    * type to the new [[org.apache.spark.angelml.linalg.Matrix]] type under the `spark.ml` package.
    *
    * @param dataset input dataset
  ...
  /**
    ...
    * type from the new [[org.apache.spark.angelml.linalg.Matrix]] type under the `spark.ml` package.
    *
    * @param dataset input dataset
  ...
  /**
    ...
    */
  private[angelml] def fastSquaredDistance(v1: Vector,
                                           norm1: Double,
                                           v2: Vector,
                                           norm2: Double,
