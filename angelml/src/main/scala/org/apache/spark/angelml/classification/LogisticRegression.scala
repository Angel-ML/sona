...
import org.apache.spark.angelml.evaluation.{BinaryClassificationMetrics, MulticlassMetrics}
...
private[classification] trait LogisticRegressionParams extends ProbabilisticClassifierParams
  ...
  import org.apache.spark.angelml.classification.LogisticRegression.supportedFamilyNames
...
class LogisticRegression @Since("1.2.0") (
  ...
  protected[spark] def train(
    ...
    val numFeatures = summarizer.mean.size.toInt
    ...
    val (coefficientMatrix, interceptVector, objectiveHistory) = {
      ...
      if ($(fitIntercept) && isConstantLabel && !usingBoundConstrainedOptimization) {
        ...
        if (initialModelIsValid) {
          ...
          if ($(fitIntercept)) {
            optInitialModel.get.interceptVector.foreachActive { (classIndex, value) =>
              initialCoefWithInterceptMatrix.update(classIndex.toInt, numFeatures, value)
            }
          }
        ...
        val interceptVec = if ($(fitIntercept) || !isMultinomial) {
          ...
          Vectors.sparse(numCoefficientSets, Seq.empty[(Int, Double)])
        }
        ...
        if ($(fitIntercept) && isMultinomial && !usingBoundConstrainedOptimization) {
          ...
          (0 until interceptVec.size.toInt).foreach { i => interceptArray(i) -= interceptMean }
        }
...
class LogisticRegressionModel private[spark] (
  ...
  private[spark] def this(uid: String, coefficients: Vector, intercept: Double) =
    this(uid, new DenseMatrix(1, coefficients.size.toInt, coefficients.toArray, isTransposed = true),
  ...
  /**
   * Returns a [[org.apache.spark.angelml.util.MLWriter]] instance for this ML instance.
   *
    ...
    override def load(path: String): LogisticRegressionModel = {
      ...
      val model = if (major.toInt < 2 || (major.toInt == 2 && minor.toInt == 0)) {
        ...
        val coefficientMatrix =
          new DenseMatrix(1, coefficients.size.toInt, coefficients.toArray, isTransposed = true)
 ...
 */
private[angelml] class MultiClassSummarizer extends Serializable {
