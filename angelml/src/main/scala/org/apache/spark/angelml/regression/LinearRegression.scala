...
import org.apache.spark.angelml.evaluation.RegressionMetrics
import org.apache.spark.angelml.feature.Instance
import org.apache.spark.angelml.linalg.BLAS._
import org.apache.spark.angelml.linalg.{Vector, Vectors}
import org.apache.spark.angelml.optim.WeightedLeastSquares
import org.apache.spark.angelml.optim.aggregator.{HuberAggregator, LeastSquaresAggregator}
import org.apache.spark.angelml.optim.loss.{L2Regularization, RDDLossFunction}
import org.apache.spark.angelml.param.shared._
import org.apache.spark.angelml.param.{DoubleParam, Param, ParamMap, ParamValidators}
...
import org.apache.spark.angelml.{PipelineStage, PredictorParams}
...
class LinearRegression @Since("1.3.0")(@Since("1.3.0") override val uid: String)
  ...
  override protected def train(dataset: Dataset[_]): LinearRegressionModel = instrumented { instr =>
    ...
    val numFeatures = dataset.select(col($(featuresCol))).first().getAs[Vector](0).size.toInt
    ...
    if (rawYStd == 0.0) {
      if ($(fitIntercept) || yMean == 0.0) {
        ...
        val coefficients = Vectors.sparse(numFeatures, Seq.empty[(Int, Double)])
        val intercept = yMean
    ...
    val (coefficients, intercept, scale, objectiveHistory) = {
      ...
      while (i < len) {
        rawCoefficients(i) *= {
          if (featuresStd(i) != 0.0) multiplier / featuresStd(i) else 0.0
        }
        i += 1
      }
...
/**
  ...
  */
@Since("1.3.0")
class LinearRegressionModel private[angelml](
  ...
  private[angelml] def this(uid: String, coefficients: Vector, intercept: Double) =
    this(uid, coefficients, intercept, 1.0)

  private var trainingSummary: Option[LinearRegressionTrainingSummary] = None

  override val numFeatures: Int = coefficients.size.toInt
  ...
  /**
    * Returns a [[org.apache.spark.angelml.util.GeneralMLWriter]] instance for this ML instance.
    *
