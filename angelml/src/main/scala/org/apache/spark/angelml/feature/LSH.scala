...
import org.apache.spark.angelml.param.{IntParam, ParamValidators}
...
private[angelml] abstract class LSHModel[T <: LSHModel[T]]
   ...
   */
  protected[angelml] def hashFunction(elems: Vector): Array[Vector]
   ...
   */
  protected[angelml] def keyDistance(x: Vector, y: Vector): Double
   ...
   */
  protected[angelml] def hashDistance(x: Seq[Vector], y: Seq[Vector]): Double
...
private[angelml] abstract class LSH[T <: LSHModel[T]]
  ...
  override def fit(dataset: Dataset[_]): T = {
    transformSchema(dataset.schema, logging = true)
    val inputDim = dataset.select(col($(inputCol))).head().get(0).asInstanceOf[Vector].size.toInt
