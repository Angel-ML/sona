...
class NaiveBayes @Since("1.5.0") (
  ...
  private[spark] def trainWithLabelCheck(
    ...
    val numFeatures = dataset.select(col($(featuresCol))).head().getAs[Vector](0).size.toInt
...
object NaiveBayes extends DefaultParamsReadable[NaiveBayes] {
  ...
  private[NaiveBayes] def requireNonnegativeValues(v: Vector): Unit = {
    val values = v match {
      case sv: IntSparseVector => sv.values
 ...
 */
@Since("1.5.0")
class NaiveBayesModel private[angelml](
    @Since("1.5.0") override val uid: String,
  ...
  @Since("1.5.0")
  override val numClasses: Int = pi.size.toInt
