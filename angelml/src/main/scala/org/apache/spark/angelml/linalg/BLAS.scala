...
private[spark] object BLAS extends Serializable {
  ...
  /**
    * y += a * x
    */
  private def axpy(a: Double, x: IntSparseVector, y: DenseVector): Unit = {
  ...
  def dot(x: Vector, y: Vector): Double = {
    ...
    (x, y) match {
      case (dx: DenseVector, dy: DenseVector) =>
        dot(dx, dy)
      case (sx: IntSparseVector, dy: DenseVector) =>
        dot(sx, dy)
  ...
  /**
    * dot(x, y)
    */
  private def dot(x: IntSparseVector, y: DenseVector): Double = {
    val xValues = x.values
    val xIndices = x.indices
    val yValues = y.values
    val nnz = xIndices.length
  ...
  /**
    * dot(x, y)
    */
  private def dot(x: IntSparseVector, y: IntSparseVector): Double = {
    val xValues = x.values
    val xIndices = x.indices
    val yValues = y.values
  ...
  /**
    ...
    */
  def dspmv(n: Int, alpha: Double, A: DenseVector, x: DenseVector,
  ...
  private def syr(alpha: Double, x: DenseVector, A: DenseMatrix) {
    ...
    nativeBLAS.dsyr("U", x.size.toInt, alpha, x.values, 1, A.values, nA)
  ...
  private def syr(alpha: Double, x: IntSparseVector, A: DenseMatrix) {
    val mA = A.numCols
    val xIndices = x.indices
    val xValues = x.values
