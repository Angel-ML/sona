...
import org.apache.spark.angelml.linalg.{JsonMatrixConverter, JsonVectorConverter, Matrix, Vector}
import org.apache.spark.angelml.util.Identifiable
...
class Param[T](val parent: String, val name: String, val doc: String, val isValid: T => Boolean)
   ...
   * columns should be implemented in [[org.apache.spark.angelml.PipelineStage.transformSchema()]].
   *
  ...
  def jValueDecode(jValue: JValue): Float = {
    jValue match {
      case JString("NaN") =>
        Float.NaN
      case JString("-Inf") =>
        Float.NegativeInfinity.toFloat
      case JString("Inf") =>
        Float.PositiveInfinity.toFloat
      case JDouble(x) =>
        x.toFloat
      case _ =>
