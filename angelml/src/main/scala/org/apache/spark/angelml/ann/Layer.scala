...
package org.apache.spark.angelml.ann
...
import org.apache.spark.angelml.optim._
...
private[ann] object ApplyInPlace {
  ...
  def apply(
    ...
    while (i < x1.rows) {
      var j = 0
      while (j < x1.cols) {
        y.update(i, j, func(x1(i, j), x2(i, j)))
        j += 1
      }
      i += 1
    }
 ...
 */
private[angelml] object FeedForwardTopology {
  /**
 ...
 */
private[angelml] class FeedForwardModel private(
    val weights: Vector,
  ...
  override def predict(data: Vector): Vector = {
    ...
    val result = forward(new BDM[Double](size.toInt, 1, data.toArray), true)
    Vectors.dense(result.last.toArray)
  }

  override def predictRaw(data: Vector): Vector = {
    val result = forward(new BDM[Double](data.size.toInt, 1, data.toArray), false)
    Vectors.dense(result(result.length - 2).toArray)
  }

  override def raw2ProbabilityInPlace(data: Vector): Vector = {
    val dataMatrix = new BDM[Double](data.size.toInt, 1, data.toArray)
 ...
 */
private[angelml] class FeedForwardTrainer(
    topology: Topology,
    val inputSize: Int,
  ...
  def train(data: RDD[(Vector, Vector)]): TopologyModel = {
    ...
    val trainData = dataStacker.stack(data)
