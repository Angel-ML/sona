...
import org.apache.spark.linalg.{DenseVector, Vector}
import com.tencent.angel.sona.ml.random.RandomDataGenerator
...
// These two classes are necessary since Range objects in Scala cannot have size > Int.MaxValue
 private[angel] class RandomRDD[T: ClassTag](sc: SparkContext,
                                              ...
                                              @transient private val seed: Long = (new Random).nextLong) extends RDD[T](sc, Nil) {
 ...
 private[angel] class RandomVectorRDD(sc: SparkContext,
                                       ...
                                       @transient private val seed: Long = (new Random).nextLong) extends RDD[linalg.Vector](sc, Nil) {
  ...
  override def compute(splitIn: Partition, context: TaskContext): Iterator[linalg.Vector] = {
 ...
 private[angel] object RandomRDD {
  ...
  def getVectorIterator(
      ...
      vectorSize: Int): Iterator[linalg.Vector] = {
    val generator = partition.generator.copy()
    generator.setSeed(partition.seed)
