...
/**
  * Params for [[LSH]].
  */
private[sona] trait LSHParams extends HasInputCol with HasOutputCol {
  /**
...
/**
  ...
  */
private[sona] abstract class LSHModel[T <: LSHModel[T]]
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val transformUDF = udf(hashFunction(_: linalg.Vector), DataTypes.createArrayType(new VectorUDT))
  ...
  private[sona] def approxNearestNeighbors(
                                            dataset: Dataset[_],
                                            key: linalg.Vector,
    ...
    val modelSubset = if (singleProbe) {
      ...
      val hashDistUDF = udf((x: Seq[linalg.Vector]) => hashDistance(x, keyHash), DataTypes.DoubleType)
    ...
    val keyDistUDF = udf((x: linalg.Vector) => keyDistance(x, key), DataTypes.DoubleType)
  ...
  def approxSimilarityJoin(
    ...
    val distUDF = udf((x: linalg.Vector, y: linalg.Vector) => keyDistance(x, y), DataTypes.DoubleType)
...
/**
  ...
  */
private[sona] abstract class LSH[T <: LSHModel[T]]
  ...
  override def fit(dataset: Dataset[_]): T = {
    transformSchema(dataset.schema, logging = true)
    val inputDim = dataset.select(col($(inputCol))).head().get(0).asInstanceOf[linalg.Vector].size.toInt
