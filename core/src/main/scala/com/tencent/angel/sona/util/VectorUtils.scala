...
object VectorUtils {
  ...
  def update(modelId: Int, func: UpdateFunc): Unit = {
  ...
  /**
    ...
    */
  def psfGet(modelId: Int, func: GetFunc): GetResult = {
    val client = MatrixClientFactory.get(modelId, PSContext.getTaskId)
  ...
  /**
    * Use ps Function(PSF) to aggregate a PSVector on PS nodes.
    */
  def psfUpdate(modelId: Int, func: UpdateFunc): Future[VoidResult] = {
    val client = MatrixClientFactory.get(modelId, PSContext.getTaskId)
  ...
  def assertCompatible(others: PSVector*): Unit = {
