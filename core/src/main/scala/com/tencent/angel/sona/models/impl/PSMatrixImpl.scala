 ...
 *
 */
package com.tencent.angel.sona.models.impl
...
import com.tencent.angel.sona.util.PSMatrixUtils
...
class PSMatrixImpl(
 ...
 checkers
  */
  def assertValid(): Unit = if (deleted) throw new AngelException(s"This Matrix has been destroyed!")
  ...
  */
  ...
  def assertRowIndexValid(rowId: Int): Unit = {
    require(rowId >= 0 && rowId < rows, s"rowId out of bound, 0 <= rowId < $rows required, $rowId given")
  }

  def assertRowIndexesValid(rowIds: Array[Int]): Unit = rowIds.foreach(assertRowIndexValid)
