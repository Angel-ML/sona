...
object ConfUtils extends Logging {
  ...
  def merge(hadoopConf: Configuration, conf: SparkConf, filterExp: String*): Configuration = {
    conf.getAll.foreach{
  ...
  def merge(sharedConf: SharedConf, params: Params, filterExp: String*): SharedConf = {
    params.extractParamMap().toSeq.foreach {
      case ParamPair(key: Param[_], value) =>
        key.name
    }
  ...
  def convertToHadoop(conf: SparkConf): Configuration = {
    ...
    ConfUtils.merge(hadoopConf, conf, "spark.ps", "spark.hadoop", "angel", "ml")
